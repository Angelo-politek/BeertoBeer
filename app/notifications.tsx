...
export default function NotificationsScreen() {
  ...
  return <ThemedView style={styles.container}><Stack.Screen options={{ title: 'Notifiche' }} /><FlatList data={items} keyExtractor={(item) => item.id} contentContainerStyle={styles.list} refreshing={loading} onRefresh={load} renderItem={({ item }) => <PressableScale onPress={() => open(item)} style={[styles.row, { backgroundColor: item.readAt ? c.surface : c.accentSoft }]}><View style={[styles.icon, { backgroundColor: c.surfaceAlt }]}><BrandIcon name={iconFor(item.category)} size={22} color={c.accent} /></View><View style={styles.text}><ThemedText type="defaultSemiBold">{item.title}</ThemedText><ThemedText style={{ color: c.textSecondary }}>{item.body}</ThemedText><ThemedText type="caption">{formatShortDate(item.createdAt)}</ThemedText></View>{!item.readAt ? <View style={[styles.dot, { backgroundColor: c.accent }]} /> : null}</PressableScale>} ListEmptyComponent={loading ? null : error ? <EmptyState icon="x-mark" title="Notifiche non caricate" message="Controlla la connessione e tira giù per riprovare." /> : <EmptyState icon="bell" title="Tutto tranquillo" message="Gli aggiornamenti importanti compariranno qui." />} /></ThemedView>;
}

...
export default function ProfileCustomizeScreen(){
  ...
  function toggleSticker(key:string){
    ...
    if(target.slot!==undefined){setValue(v=>({...v,stickers:v.stickers.map(s=>s.key===key?{...s,slot:undefined}:s)}));return;}
    ...
    const used=new Set(value.stickers.filter(s=>s.slot!==undefined).map(s=>s.slot));

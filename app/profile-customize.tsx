...
export default function ProfileCustomizeScreen(){
  ...
  function toggleSticker(key:string){setValue(v=>{const selected=v.stickers.filter(s=>s.slot!==undefined);return {...v,stickers:v.stickers.map(s=>s.key!==key?s:s.slot!==undefined?{...s,slot:undefined}:{...s,slot:selected.length<3?selected.length:undefined})};});}
  ...
  async function save(){setSaving(true);try{await saveProfileCustomization(value,value.stickers);router.back();}catch{Alert.alert('Salvataggio non riuscito','Controlla i dati e riprova.');setSaving(false);}}
  const preview=useMemo(()=>({...value,stickers:value.stickers.filter(s=>s.slot!==undefined)}),[value]);
...
function StickerRow({sticker,onToggle,onTune}:{sticker:ProfileSticker;onToggle:()=>void;onTune:(key:string,k:'scale'|'rotation',d:number)=>void}){const c=useColors();const active=sticker.slot!==undefined;return <Card style={[styles.stickerRow,{opacity:sticker.unlocked?1:.55}]}><ProfileStickerImage assetKey={sticker.assetKey}/><View style={{flex:1}}><ThemedText type="subtitle">{sticker.title}</ThemedText><ThemedText type="caption">{sticker.unlocked?sticker.description:sticker.unlockHint}</ThemedText>{active?<View style={styles.tuners}><Chip label="Più piccolo" active={false} onPress={()=>onTune(sticker.key,'scale',-.1)}/><Chip label="Ruota" active={false} onPress={()=>onTune(sticker.key,'rotation',6)}/></View>:null}</View><Pressable disabled={!sticker.unlocked} onPress={onToggle} style={[styles.select,{backgroundColor:active?c.accent:c.surfaceAlt}]}><BrandIcon name={active?'check':'plus'} size={18} color={active?c.accentText:c.text}/></Pressable></Card>}
